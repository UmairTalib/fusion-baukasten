...
function setCoreInteractionLocked(locked) {
  const shouldLock = locked || toolkitGenerated;
  coreInput.disabled = shouldLock;
  coreSend.disabled = shouldLock;
  coreNext.disabled = shouldLock;
  document.querySelectorAll("[data-core-choice], [data-core-tool]").forEach((button) => {
    button.disabled = shouldLock;
  });
}

async function addCoreReply(text, strongText = "", delay = 720) {
  if (coreReplyPending) return false;
  coreReplyPending = true;
  setCoreInteractionLocked(true);

  const indicator = document.createElement("article");
  indicator.className = "dialogue-message ai-message typing-message";
  indicator.setAttribute("aria-label", "Fusion tippt");
  indicator.innerHTML = `
    <div class="dialogue-avatar">AI</div>
    <div class="dialogue-card typing-card">
      <span class="typing-label">Fusion tippt</span>
      <span class="typing-dots" aria-hidden="true"><i></i><i></i><i></i></span>
    </div>
  `;
  coreDialogueStream.append(indicator);
  coreDialogueStream.scrollTop = coreDialogueStream.scrollHeight;
  ...
  await wait(140);
  indicator.remove();
  addCoreMessage(text, "ai", strongText);

  coreReplyPending = false;
  setCoreInteractionLocked(false);
  return true;
}
...
function updateCoreProgress() {
  ...
  coreProgressBar.classList.remove("is-updating");
  coreProgressText.classList.remove("is-updating");
  window.requestAnimationFrame(() => {
    coreProgressBar.classList.add("is-updating");
    coreProgressText.classList.add("is-updating");
  });
  window.setTimeout(() => {
    coreProgressBar.classList.remove("is-updating");
    coreProgressText.classList.remove("is-updating");
...
async function generateToolkitFromCore(options = {}) {
  ...
  await addCoreReply(
    "Der Core Flow ist abgeschlossen. Ich habe den Werkzeugkasten vorbereitet.",
    "Du kannst jetzt zum Werkzeugkasten wechseln oder die Ergebnisse herunterladen.",
    options.replyDelay ?? 760,
  );
  showToast("Werkzeugkasten wurde generiert.");
}

async function advanceCoreFlow(answer, options = {}) {
  if (coreReplyPending) return;
  const cleanAnswer = answer.trim();
  if (!cleanAnswer && !options.force) return;
  if (toolkitGenerated) {
    await addCoreReply("Der Core Flow ist bereits abgeschlossen. Ich kann dich jetzt zum Werkzeugkasten bringen.");
    return;
  }
  ...
  if (coreCurrentStep >= coreSteps.length - 1) {
    await generateToolkitFromCore(options);
    return;
  }
  ...
  await addCoreReply(
    `Danke, ich habe Block ${coreCurrentStep} gespeichert. Als nächstes geht es um „${step.title}“.`,
    step.question,
    options.replyDelay ?? 720,
  );
  coreInput.value = "";
}

async function goBackCoreStep() {
  if (coreReplyPending) return;
  ...
  coreCurrentStep -= 1;
  updateCoreProgress();
  await addCoreReply(`Ich bin zurück zu „${coreSteps[coreCurrentStep].title}“.`, coreSteps[coreCurrentStep].question, 560);
}
...
async function runDemoFlow() {
  ...
  await addCoreReply("Ich führe dich jetzt im Schnelllauf durch den Core Flow.", "Danach übergebe ich die Ergebnisse an Aufgaben, Kalender, Werkzeugkasten und Reporting.", 620);
  await wait(250);
  ...
  for (const answer of demoAnswers) {
    ...
    await advanceCoreFlow(answer, { replyDelay: 520 });
    await wait(180);
  }

  if (!toolkitGenerated) {
    await generateToolkitFromCore({ replyDelay: 600 });
  }
...
document.querySelectorAll("[data-core-tool]").forEach((button) => {
  button.addEventListener("click", async () => {
    ...
    const step = coreSteps[coreCurrentStep];
    if (tool === "examples") {
      await addCoreReply("Hier ist ein Beispiel, das du anpassen kannst:", step.hint, 520);
      coreInput.value = step.hint.replace("Beispiel: ", "");
      coreInput.focus();
      return;
    }
    if (tool === "explain") {
      await addCoreReply(`Warum frage ich das? Dieser Block hilft Fusion, später passende Empfehlungen, Aufgaben und Checklisten für „${step.title}“ zu erzeugen.`, "", 600);
      return;
    }
    if (tool === "suggest") {
      coreInput.value = step.hint.replace("Beispiel: ", "");
      await addCoreReply("Ich habe einen Vorschlag in das Eingabefeld gelegt. Du kannst ihn übernehmen oder bearbeiten.", "", 560);
      coreInput.focus();
    }
  });
});
...
document.addEventListener("click", (event) => {
  ...
  if (view === "coreflow") {
    if (label.includes("Datei")) addCoreReply("Datei wurde als Kontext hinzugefügt.", "Workshop_Dokument_v2.docx ist jetzt mit dem Core Flow verbunden.", 620);
    if (label.includes("Websuche")) addCoreReply("Websuche ergänzt.", "3 externe Inspirationsquellen wurden als Kontext markiert.", 620);
    if (label.includes("Dokument")) addCoreReply("Dokument analysiert.", "Ich habe Zielgruppen, Risiken und Aufgaben aus dem Dokument erkannt.", 680);
    if (label.includes("Kontext")) addCoreReply("Kontext hinzugefügt.", "Der aktuelle Projektkontext wurde erweitert.", 560);
    if (label.includes("Zwischenstand")) addCoreReply("Zwischenstand geteilt.", "Ein Review-Link wurde für das Team vorbereitet.", 620);
    if (label.includes("Feedback")) addCoreReply("Feedback-Prozess aktualisiert.", "Feedback wurde angefragt oder in den Dialog integriert.", 620);
    return;
  }
